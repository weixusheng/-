...
var url='../data/data.php?grade_code='+grade_code+'&n='+Math.random();//设定URL传值方法同时防止缓存
...
createXMLHttpRequest();//调用创建XmlHttp的函数
type="term";//查询县的
var url="../data/data1.php?class_name="+class_name+'&n='+Math.random();//设定URL传值方法同时防止缓存
url=encodeURI(url);
...
createXMLHttpRequest();//调用创建XmlHttp的函数
type="subject";//查询县的
var url="../data/data2.php?term="+term+'&classname='+classname+'&n='+Math.random();//设定URL传值方法同时防止缓存
url=encodeURI(url);
xmlHttp.open("GET",url,true);
xmlHttp.onreadystatechange=handleStateChange;//处理响应的函数名
xmlHttp.send(null);//执行程序函数这里的中间的参数是因为GET原因
...
function dd() {
	if(document.form1.grade_code.value==-1){
		alert("请选择年级");
		form1.grade_code.focus();
		return false;
	}
	if(document.form1.class_name.value==-1){
		alert("请选择班级");
...
$sql = ("select distinct tb_class.grade_code from tb_class,tb_course2_term,tb_teacher_base where tb_class.graduate_flag='0' and tb_class.grade_code=tb_course2_term.grade and tb_course2_term.c_teacher=tb_teacher_base.t_name and tb_teacher_base.t_no='$t_no'");//查询数据库年级
$result = mysql_query($sql);//执行语句赋值给变量
?>
<form name="form1" method="post" action="show_by_course1.php" target="window" onsubmit="return dd()"><!--输出表单头-->
<!--输出下拉列表框，并设定onchange响应事件，把年级值传递过去-->
<select id="grade_code" name="grade_code" onchange='queryGrade(this.options[this.selectedIndex].value)'>
<!--输出下拉列表项值-->
...
<select name="class_name" id="class_name" onchange='queryClass(this.options[this.selectedIndex].value)'></select> 
<select name="term" id="term" onchange="querySub(this.options[this.selectedIndex].value,form1.class_name.options[class_name.selectedIndex].value)"></select>
