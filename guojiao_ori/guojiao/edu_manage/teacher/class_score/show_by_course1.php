...
$sql_as=mysql_query("select ID,c_week_hour,c_teacher from tb_course2_term where cb_id='$cb_id' and c_class='$class' and term='$term';");
$info_as=mysql_fetch_array($sql_as);
$c_week_hour=$info_as["c_week_hour"];
$c_teacher=$info_as["c_teacher"];
$c_id=$info_as["ID"];
?>
<input type="button" name="button1" value="导为Excel" onclick="location.href='../export_course_score.php?term=<?php echo $term;?>&classes0=<?php echo $class_id?>&cb_id=<?php echo trim(substr($cb_id,0,(strlen($cb_id)-2)));?>&way=<?php echo $way; ?>'" />
<!--startprint-->
...
年级:［<?php echo $grade_code; ?>］学院:［国际交流学院］专业:［<?php echo $major; ?>］班级:［<?php echo $class; ?>］学年学期:［<?php echo $term; ?>］<br />
</caption>
<?php  
...
switch ($way){
	case '0':
			$asql=mysql_query("select count(ID) as all_num from tb_s_information where now_class='$class' and (stu_status='0' or stu_status='4');");
