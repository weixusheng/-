...
if($way==0){
	...
	if(substr($major_00,0,1)=='E'){
		...
		$part10=$info12["part"];
		$sql13=mysql_query("select count(ID) as part from tb_score where s_term='$term' and s_id in (select ID from tb_s_information where now_class='$class_name' and (stu_status=0 or stu_status=4)) and  c_id='$c_id' and score=0 and score is not NULL and exam_type=0;");
...
$sql25=mysql_query("select s_id,score,bk_score from tb_score where s_term='$term' and s_id in (select ID from tb_s_information where now_class='$class_name') and c_id='$c_id';");
for($i=6;$i<36;$i++){
