...
<?php
	...
	if($exam_temp[0]=='score'){$verify_type="verify";$exam_aud='exam_type';}
	else {
		$verify_type=$exam_temp[0]."_verify";
...
<form name="form" action="auditing_score_save.php?class_name=<?php echo $class_name;?>&id=<?php echo $id;?>&exam=<?php echo $exam_type;?>&verify=<?php echo $verify_type;?>" method="post" onsubmit="document.getElementById('all_table').innerHTML='<img src=\'../../images/loading.gif\' /><font size=\'2\' color=\'#FF0000\'>正在处理，请耐心等待</font>';document.getElementById('all_table').align='left'">
    <table width="65%" border="1" cellspacing="0" cellpadding="0" >
      <tr>
        ...
        <td width="25%" align="center">全部审核
          <input type="checkbox" name="shAll" value="" id="shAll" onClick="ChkAllClick('shSon[]','shAll')" /></td>
      </tr>
      <?php
    $sql=mysql_query("select ID,s_id,$exam_type,$verify_type,$exam_aud from tb_score where c_id='$c_id' and ($exam_type is not null  or $exam_aud=1)");
    ...
    ?>
      <tr>
        ...
        <td align="center"><?php 
          ...
          <td align="center">
                <input <?php if (!(strcmp($verify,1))) {echo "checked=\"checked\" disabled=\"disabled\" ";} ?> type="checkbox" name="shSon[]" value=<?php echo $score_id;?> id="" onclick="ChkSonClick('shSon[]','shAll');change_submit(false)" />
                &nbsp;</td>
      </tr>
      <?php
