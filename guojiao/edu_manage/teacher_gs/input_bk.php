...
$info=mysql_fetch_array($sql);
	do{
		...
		<td>";
		if($info["ck3_score"]!=NULL){
			echo "<input type='text' size='3' name='grade[]' value='$info[ck3_score]'  readonly='readonly' onfocus='tishi()' />";
		}else{
			echo "<input type='text' size='3' name='grade[]' value='' />";
		}
		echo "<input type='hidden' name='score_id[]' value='$info[ID]'></td></tr>";
	$i++;
	}while($info=mysql_fetch_array($sql));
	echo "<tr><td colspan='6'><div style='padding:0px 10px;'><input type='submit' value='确认提交'></div></td></tr>";	
}else{
	echo "<tr align='center'><td colspan='6'>无补考名单</td></tr>";
	}
?>
<input  type="hidden" value="添加" name="way" />
<input  type="hidden" value="input" name="way1" />
</table>
</form>
</body>
</html>