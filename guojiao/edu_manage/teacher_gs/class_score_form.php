...
var url='../teaching_secretary/cjgl/data.php?grade_code='+grade_code+'&n='+Math.random();//设定URL传值方法同时防止缓存
...
createXMLHttpRequest();//调用创建XmlHttp的函数
type="term";//查询县的
var url="../teaching_secretary/cjgl/data1.php?class_name="+class_name+'&n='+Math.random();//设定URL传值方法同时防止缓存
url=encodeURI(url);
...
createXMLHttpRequest();//调用创建XmlHttp的函数
type="subject";//查询县的
var url="../teaching_secretary/cjgl/data2.php?term="+term+'&classname='+classname+'&n='+Math.random();//设定URL传值方法同时防止缓存
url=encodeURI(url);
xmlHttp.open("GET",url,true);
xmlHttp.onreadystatechange=handleStateChange;//处理响应的函数名
xmlHttp.send(null);//执行程序函数这里的中间的参数是因为GET原因
...
?>
<form name="form1" method="post" action="export_course_score.php" onsubmit="return f_tijao()"><!--输出表单头-->
