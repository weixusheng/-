   ...
   //生成目标文件的文件名
		...
		if (move_uploaded_file($_FILES["file"]["tmp_name"],$uploadfile))
		{
			if(!is_uploaded_file($_FILES["file"]["tmp_name"]))
			{
				...
				$hsql=mysql_query("select credit,cb_id,term from tb_course2_term where ID='$c_id'");
				...
				if($type0==0){
				  ...
				  $sql="INSERT INTO tb_score(s_id,c_id,c_credit,s_term,score) VALUES('".$ginfo["ID"]."','".$c_id."','".$hinfo["credit"]."','".$hinfo["term"]."','".trim($data->sheets[0]['cells'][$i][$j])."')";
				  $res = mysql_query($sql);
				  }
				}
				//else{
