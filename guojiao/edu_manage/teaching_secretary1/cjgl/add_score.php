...
<script type="text/javascript">
	function f_tijao() {
		...
		if (form1.stu_name.value == -1) {
			alert("请选择学生");
			form1.stu_name.focus();
			return false;
		}
		...
		if(form1.real_score.value==""){
			alert("请输入成绩");
			form1.real_score.focus();
			return false;
			}
		return true;
	}
...
function queryStu(class_name){
eval("document.form1.stu_name.length=0;");//eval检查 JScript 代码并执行. 
eval("document.form1.stu_name.options.add(new Option('请选择学生','-1'));");
createXMLHttpRequest();//调用创建XmlHttp的函数
type="stu_name";//查询县的
var url="data3.php?class_name="+class_name+'&n='+Math.random();//设定URL传值方法同时防止缓存
url=encodeURI(url);
xmlHttp.open("GET",url,true);
xmlHttp.onreadystatechange=handleStateChange;//处理响应的函数名
xmlHttp.send(null);//执行程序函数这里的中间的参数是因为GET原因
...
{
   aa = a[i].split('**');
   eval("document.form1.stu_name.options.add(new Option(aa[1]+'--'+aa[2],aa[0]));");//eval检查 JScript 代码并执行. 
}
}
</script>
...
<body class="bgc" style="padding: 50px 0 0 50px">
<?php
...
?>
<form name="form1" method="post" action="add_score_save.php"
	target="_self" onsubmit="return f_tijao()"><!--输出表单头--> <!--输出下拉列表框，并设定onchange响应事件，把年级值传递过去-->
<select id="grade_code" name="grade_code"
	onchange='queryGrade(this.options[this.selectedIndex].value)'>
	<!--输出下拉列表项值-->
...
<select name="class_name" id="class_name"
	onchange='queryStu(this.options[this.selectedIndex].value)'>
</select>

<select name="stu_name" id="stu_name" onchange="queryClass(class_name.options[class_name.selectedIndex].value);"></select>
<select name="term" id="term"
	onchange="querySub(this.options[this.selectedIndex].value,form1.class_name.options[class_name.selectedIndex].value)"></select>
<select name="subject"></select> <select name="exam">
	<option value="score">正考</option>
	<option value="bk_score">补考</option>
	<option value="ck1_score">重修1</option>
	<option value="ck2_score">重修2</option>
	<option value="ck3_score">重修3</option>
</select> 
...
&nbsp;&nbsp;是否评教：<input  type="checkbox"  name="evaluate" value="1" />&nbsp;&nbsp;成绩：<input name="real_score" type="text"></input>
<input name="" type="submit" value="确定" /></form>
</body>
</html>